...
def require_nvidia_key() -> str:
    if not NVIDIA_API_KEY:
        raise RuntimeError(
            "NVIDIA_API_KEY is not set. Local mode needs an NVIDIA key when LLM_PROVIDER=nvidia."
        )
    return NVIDIA_API_KEY


_viral_weights_env = os.getenv("VIRAL_SCORE_WEIGHTS", "")
if _viral_weights_env:
    import json
    VIRAL_SCORE_WEIGHTS = json.loads(_viral_weights_env)
else:
    VIRAL_SCORE_WEIGHTS = {
        "hook_strength": 1.0,
        "curiosity_gap": 1.0,
        ...
        "conflict": 1.0,
        "surprise": 1.0,
        "authority_credibility": 1.0,
        "viewer_retention_prediction": 1.0,
        "shareability": 1.0,
        "comment_potential": 1.0,
        "story_completion_score": 1.0,
        "emotion_score": 1.0
    }
